...
# VOC (Voice of the Customer) 데이터셋 처리 클래스
class VOCDataset:
    ...
    def __init__(self, root, transform=None, target_transform=None, is_test=False, keep_difficult=False, label_file=None):
        ...
        # 해당 파일이 존재하지 않으면 대체 defualt.txt 파일 확인 else 에러    
        ...
        # class명과 index를 매핑한 딕셔너리 생성
        self.class_dict = {class_name: i for i, class_name in enumerate(self.class_names)}

    # 2. 데이터셋 초기화
    # 데이터셋에서 특정 인덱스의 이미지, 바운딩 박스, 레이블을 가져오는 메서드
    ...
    # 주어진 인덱스의 이미지 ID와 해당 이미지의 바운딩 박스, 클래스 레이블을 반환
    ...
    # 4. 이미지 ID 및 어노테이션 처리 메서드
    # 주어진 이미지 세트 파일에서 이미지 ID를 읽어옴
    ...
    # 주어진 이미지 ID의 어노테이션 수 반환
    def _get_num_annotations(self, image_id):
        annotation_file = self.root / f"Annotations/{image_id}.xml"
        objects = ET.parse(annotation_file).findall("object")
        return len(objects)
        
    # 주어진 이미지 ID에 대한 바운딩 박스, 클래스 레이블, 어려운(difficult) 객체 여부를 반환
